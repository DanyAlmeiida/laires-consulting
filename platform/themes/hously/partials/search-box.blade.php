<div @class(['grid grid-cols-1', 'mt-10' => $style === 2])>
    ...
    <div @class([
        ...
        'max-h-[500px] overflow-y-auto' => $style == 4,
    ])>
        @foreach($searchTabs as $key => $value)
            @continue(! in_array($key, $shortcode->search_tabs))
            <div @class(['hidden' => (! $loop->first) || (! request()->query('type') == $key)]) id="{{ $key }}" role="tabpanel" aria-labelledby="{{ $key }}-tab">
                @php($type = $key === 'projects' ? 'project' : 'property')
                {!! Theme::partial("filters.$type", ['type' => $key, 'categories' => $categories, 'style' => $style]) !!}
            </div>
        @endforeach
    </div>
</div>
