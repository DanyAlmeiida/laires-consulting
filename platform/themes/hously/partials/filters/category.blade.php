<div>
    ...
    <div class="relative mt-2 filter-search-form filter-border">
        ...
        <select class="form-select z-2" data-trigger name="category_id" data-choices-js id="choices-category-{{ $id ?? $type }}" aria-label="{{ __('Category') }}">
            <option value="">{{ __('All Category') }}</option>
            @foreach($categories as $category)
                <option @selected(request()->query('category_id') ==  $category->id) value="{{ $category->id }}">{{ $category->name }}</option>
            @endforeach
        </select>
    </div>
</div>
