<form action="{{ $actionUrl ?? RealEstateHelper::getPropertiesListPageUrl() }}" class="search-filter search-ajax-item" data-ajax-url="{{ $ajaxUrl ?? route('public.properties') }}">
    ...
    <div class="space-y-5 registration-form text-dark text-start">
        ...
        <div class="hidden space-y-5 transition-all duration-200 ease-in-out advanced-search">
            <div @class(['grid grid-cols-1 gap-6 lg:gap-0', ' lg:grid-cols-2' => $style == 4, 'md:grid-cols-2 lg:grid-cols-4' => $style !== 4])>
                @php $id = null; @endphp
