<div>
    ...
    <div class="relative mt-2 filter-search-form filter-border">
        ...
        <select data-trigger name="bathroom" data-choices-js id="choices-bathrooms-{{ $id ?? $type }}" aria-label="{{ __('Bathrooms') }}">
            <option value="">{{ __('All Bathrooms') }}</option>
            @foreach(range(1, 5) as $i)
                <option @selected(request()->query('bathroom') == $i) value="{{ $i }}">
                    @if($i !== 5)
