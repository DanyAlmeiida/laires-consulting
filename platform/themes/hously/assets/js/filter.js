...
class ItemFilter {
    ...
    constructor() {
        ...
        if ($('.item-search.search-ajax').length) {
            const $searchAjaxItem = $('form.search-ajax-item')

            $searchAjaxItem.on('submit', (e) => {
                e.preventDefault()
                ...
                const url = searchParams.toString() === '' ? $form.data('ajax-url') : `${$form.data('ajax-url')}?${searchParams.toString()}`
                ...
                if (searchParams.toString().length) {
                    $form.find('.reset-filter').removeClass('hidden')
                }
            })

            $searchAjaxItem.on('click', '.reset-filter', (e) => {
                e.preventDefault()
                ...
                $form.not(':has(> input[name="type"])') .find('input').val('')
                $form.find('select').prop('selectedIndex', 0)
                this.refresh($form.data('ajax-url'))
                $form.find('.reset-filter').addClass('hidden')
                window.history.pushState({}, '', $form.data('ajax-url'))
            })
        ...
        this.$itemElement
            ...
            .on('click', '.reset-filter', (e) => {
                ...
                this.refresh()
                this.$itemElement.find('.reset-filter').addClass('hidden')
            })
    ...
    refresh(url = null) {
        ...
        setTimeout(() => {
            $.ajax({
                url: url || this.ajaxUrlWithQueryString,
                type: 'POST',
                success: (res) => {
    ...
    updateBrowserUrl() {
        ...
        if (this.searchParams.toString().length) {
            this.$itemElement.find('.reset-filter').removeClass('hidden')
        }
    }
