...
class SearchFilter {
    init() {
        ...
        let isFilteredByType = false
        const searchElement = $('.item-search.search-ajax')
        if (searchElement.length) {
            isFilteredByType = searchElement.data('target') === 'property' && urlParameters.get('type')

            if (isFilteredByType) {
                $('#' + urlParameters.get('type') + '-tab').click()
            }

            const target = $('.item-search.search-ajax').data('target')
            const $searchTab = $('#searchTab')
            const searchFilter = $('.search-filter')
            if (target === 'property') {
                $searchTab.find('#projects-tab').remove()
                searchFilter.find('#projects').remove()
                const tabToggle = $('#sale-tab') || $('#rent-tab')
                tabToggle.click()
